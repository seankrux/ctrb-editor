...
// ── Tool: lookup_gmb ─────────────────────────────────────────────────────────
server.tool('lookup_gmb', 'Find a brand\'s Google My Business listing. Returns CID (→AuthorId), place_id, kg_id, profile URL (→TargetUrl), and coordinates.', {
...
server.tool('lookup_gmb_nearby', `Find a brand GMB + nearby location CIDs + generate unique GMB URLs.
  ...
  - ctr_booster_config: ready-to-use AuthorId, TargetUrl, lstCustomGeolocations`, {
    ...
    process.stderr.write(`[lookup_gmb_nearby] Found brand: ${brand.name} | CID: ${brand.cid} | ` +
        `lat: ${brand.latitude}, lng: ${brand.longitude}\n`);
    // Step 2: Find nearby businesses
    process.stderr.write(`[lookup_gmb_nearby] Step 2: Searching ${count} nearby "${nearby_category ?? 'business'}" locations...\n`);
    const nearbyBrands = await searchNearby(brand.latitude, brand.longitude, nearby_category ?? 'business', count);
    process.stderr.write(`[lookup_gmb_nearby] Found ${nearbyBrands.length} nearby locations\n`);
...
server.tool('bulk_lookup_gmb', 'Look up multiple brands in a single batch via Pleper API. Returns CID, place_id, and GMB URL for each.', {
    ...
    const geocoded = await Promise.all(brands.map(async (b) => {
...
server.tool('get_profile_info', 'Get full GMB profile details by Google Maps URL or CID. Returns all fields including hours, images count, reviews summary.', {
    profile_url: z.string().describe('Google Maps URL or CID URL, e.g. "https://www.google.com/maps?cid=1234567890" or "https://www.google.com/maps/place/..."'),
}, async ({ profile_url }) => {
...
// ── Tool: build_gmb_url ──────────────────────────────────────────────────────
server.tool('build_gmb_url', 'Build a Google Maps URL that loads a brand\'s GMB listing from a specific nearby lat/lng viewpoint.', {
    brand_name: z.string(),
    ...
    }, input.viewport_lat, input.viewport_lng, input.zoom ?? 14);
    return {
        content: [{
                type: 'text',
