...
async function testEndpoint(label, url, params) {
    console.log(`\n--- ${label} ---`);
    try {
...
async function main() {
    ...
    await testEndpoint('Maps search with query', `${BASE}/google/search/maps`, { ...auth, batch_id: batchId, query: "McDonald's Austin TX", language: 'en' });
    await testEndpoint('Maps search with keyword', `${BASE}/google/search/maps`, { ...auth, batch_id: batchId, keyword: "McDonald's Austin TX" });
    await testEndpoint('Profile info by CID URL', `${BASE}/google/by-profile/information`, { ...auth, batch_id: batchId, profile_url: 'https://www.google.com/maps?cid=5766352990099467929' });
    // Commit
