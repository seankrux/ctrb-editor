...
function toForm(data) {
    return new URLSearchParams(Object.entries(data).reduce((a, [k, v]) => ({ ...a, [k]: String(v) }), {})).toString();
}
...
async function runBatch(jobs) {
    ...
    await Promise.all(jobs.map(j => http.post(`/${j.endpoint}`, toForm({ ...auth(), ...j.formData, batch_id: batchId }), {
...
export async function geocodeCity(location) {
    try {
        ...
        process.stderr.write(`[geocode] Error for "${location}": ${e.message}\n`);
    }
    return null;
}
// ── Public API ────────────────────────────────────────────────────────────────
 ...
 */
export async function searchBrand(brandName, location) {
    // Geocode city first
 ...
 */
export async function bulkSearchBrands(items) {
 ...
 */
function spreadOffsets(lat, lng, count) {
