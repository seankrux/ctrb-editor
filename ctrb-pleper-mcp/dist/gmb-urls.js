 ...
 */
export function buildBrandUrlFromNearby(brand, nearbyLat, nearbyLng, zoomLevel = 14) {
 ...
 */
export function buildGmbResult(brand, nearbyBusinesses) {
    const nearby = nearbyBusinesses.map((nb, i) => ({
        index: i + 1,
        name: nb.name,
        cid: nb.cid,
 ...
 */
function generateScatterPoints(lat, lng, count, radiusMiles = 5) {
